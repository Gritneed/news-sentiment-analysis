...
# Add internal backend modules to path
...
def health_check():
    ...
    """
    return jsonify(
        {"status": "ok", "message": "News Sentiment Analyzer API is running"}
    )
...
def analyze_company():
    """
    Analyze sentiment for a given company based on recent news articles.
    Expects JSON input with 'company' and optional 'days_back'.
    """
    data = request.json
